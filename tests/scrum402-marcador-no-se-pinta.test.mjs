...
const CENSO = Object.freeze({
  ...
  'invoicesView.js': 1,
  // 🔴 SCRUM-606 (ALB-01) · 5-sep-2026 · ENTRARON DOS FICHEROS Y SALIERON EL MISMO DÍA.
  //
  // `albaranDesdePresupuestoModal.js` entró con 1 (seis textos tras una sola constante `MARCA`)
  // y `atajoNuevo.js` con 1 (el rótulo de la cuarta lista, que su propio comentario había
  // predicho el 4-sep). El asesor firmó los siete esa misma tarde —cuatro tal cual, dos con
  // cambio y uno acortado antes de enseñárselo—, todos CON LA CAJA MEDIDA delante a 929 y 390 px.
  //
  // Sus entradas se BORRAN y no se ponen a 0 (SCRUM-424 / SCRUM-405): `censoActual()` sólo lista
  // ficheros CON marcadores, así que el trinquete APRIETA. Y con ellas se retiró `MARCA` entera
  // del modal —precedente de `MARCA_651`—: una constante de marcado sin consumidores invita a
  // marcar texto nuevo en vez de someterlo. El registro, en `docs/master/SCRUM-606.md`.
