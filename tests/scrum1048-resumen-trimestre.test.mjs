...
import {
  agruparIvaPorTipo,
  calcularIvaSoportado,
  construirResumenTrimestre,
  retencionesNoDisponibles,
} from '../dist/modules/reports/domain/resumenTrimestre.js';
...
test('retenciones: SIEMPRE no disponibles — Invoice no guarda la retención aplicada (SCRUM-293/A2)', () => {
  const r = retencionesNoDisponibles();
  assert.equal(r.disponible, false);
  assert.match(r.motivo, /ALTER/, 'el motivo tiene que decir POR QUÉ, no solo que falta');
...
test('agruparIvaPorTipo: mismo tipo en dos entradas se suma, orden descendente por tipo', () => {
  const r = agruparIvaPorTipo([
