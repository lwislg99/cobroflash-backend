...
export const MUTACIONES_QUE_ME_TUMBAN = [
  ...
  },
  {
    ...
    // ⚠️ ANCLA ACTUALIZADA EN SCRUM-834b: SCRUM-824b (`el rojo intermitente era un sha corto que
    // parecía un número`) partió el `if` de una línea en dos —`ES_SHA` aparte de
    // `TODO_DIGITOS`/`LONGITUDES_DE_RELOJ`— y dejó esta declaración apuntando a un texto que ya
    // no existe: el meta-guard salía CIEGO, no MUDO, porque el ancla había caducado.
    fichero: 'scripts/_ritmo-de-despliegue.mjs',
