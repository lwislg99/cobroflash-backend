// SCRUM-13 (COBROS-1): recalcJobCobradoForCharge — suma desde cero + idempotencia.
// Prueba que un pago duplicado (mismo Charge paid, webhook 2×) NO duplica totalCobrado,
// y que el semáforo (Parcial→Pagado) sale de sumar los Charge paid del Job.
//
...
test('SCRUM-13: recalcJobCobradoForCharge — suma desde cero, IDEMPOTENTE', { skip: !ENABLED }, async () => {
  const { prisma } = await import('../dist/core/db/prisma.js');
  const { recalcJobCobradoForCharge, estadoCobroFor } = await import('../dist/modules/jobs/domain/job.service.js');
  ...
  try {
    ...
    // 2 tramos 50/50: A pagado, B pendiente (cada tramo = 1 Charge + 1 Invoice, como en prod)
    ...
    await prisma.invoice.create({ data: { merchantId: MERCHANT_ID, customerId: customer.id, quoteId: quote.id, chargeId: chargeA.id, number: `J-QA13A-${Date.now()}`, total: '50.00', currency: 'EUR', pdfUrl: 'PENDING_PDF', qrData: 'PENDING_QR', clientComment: MARK, status: 'paid' } });
    await prisma.invoice.create({ data: { merchantId: MERCHANT_ID, customerId: customer.id, quoteId: quote.id, chargeId: chargeB.id, number: `J-QA13B-${Date.now()}`, total: '50.00', currency: 'EUR', pdfUrl: 'PENDING_PDF', qrData: 'PENDING_QR', clientComment: MARK, status: 'pending' } });
    ...
    // 1er recálculo (tramo A pagado) → 50 (Parcial)
    await recalcJobCobradoForCharge(chargeA.id);
    let j = await prisma.job.findUnique({ where: { id: job.id }, select: { totalCobrado: true } });
    assert.equal(Number(j.totalCobrado), 50, 'tras pagar el 1er tramo, totalCobrado = 50 (Parcial)');
    assert.equal(estadoCobroFor(Number(j.totalCobrado), 100), 'Parcial', 'semáforo = Parcial con 50/100');

    // IDEMPOTENCIA: mismo pago otra vez (webhook duplicado) → SIGUE 50, no 100
    await recalcJobCobradoForCharge(chargeA.id);
    j = await prisma.job.findUnique({ where: { id: job.id }, select: { totalCobrado: true } });
    assert.equal(Number(j.totalCobrado), 50, 'IDEMPOTENTE: webhook duplicado NO duplica (sigue 50)');

    // Pagar el 2º tramo → recálculo → 100 (Pagado)
    await prisma.charge.update({ where: { id: chargeB.id }, data: { status: 'paid' } });
    await recalcJobCobradoForCharge(chargeB.id);
    j = await prisma.job.findUnique({ where: { id: job.id }, select: { totalCobrado: true } });
    assert.equal(Number(j.totalCobrado), 100, 'tras pagar el 2º tramo, totalCobrado = 100 (Pagado)');
    ...
    console.log('✔ SCRUM-13: 50/Parcial → (duplicado) 50 → 100/Pagado. Suma desde cero e idempotente.');
  } finally {
    await cleanup();
