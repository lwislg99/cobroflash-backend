...
//
// `wipeDemo` borraba **10 de los 23 modelos con `merchantId`**. Los once que faltaban —medidos
...
test('SCRUM-314 · CONTROL: ningún borrado sale sin filtro del demo', async () => {
  ...
  // como la cobertura: sin él, «cubrir los 23» se podría lograr borrando las tablas enteras.
  const p = prismaEspia();
  const { modelos } = await barridoDemo(p, DEMO_ID);
  ...
  for (const { modelo, where } of p.borrados) {
    ...
    const acotado =
      where?.merchantId === DEMO_ID ||                       // los 23 por merchant
