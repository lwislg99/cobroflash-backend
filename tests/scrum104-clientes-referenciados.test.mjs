...
test('SCRUM-104 (fase 2): clientes.csv del paquete = los referenciados, ni más ni menos', { skip: !ENABLED && 'sin QA_DB_TEST=1 · npm run test:staging:gated' }, async (t) => {
  ...
  // SCRUM-113: el merchant y TODO su montaje dentro de withMerchant. Antes se creaba
  // fuera del try: si reventaba el alta de un cliente o de una factura, el finally ni se
  // planteaba y quedaba huérfano en staging.
  try {
    await withMerchant(prisma, {
      name: `QA S104 ${stamp}`, email: `qa-s104-${stamp}@test.local`, invoiceSeriesPrefix: 'CF',
    }, async (merchant) => {
