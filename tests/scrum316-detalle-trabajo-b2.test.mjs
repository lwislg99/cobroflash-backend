...
test('SCRUM-316 · la vista PINTA desde el registro, no a mano', () => {
  ...
  const apendices = (VISTA.match(/headRight\.appendChild/g) || []).length;
  const enEnsamblado = (VISTA.match(/cubosCabecera\.\w+\.forEach\(\(b\) => headRight\.appendChild/g) || []).length;
  assert.ok(
    apendices > 0,
  ...
  // Todo appendChild sobre headRight vive en el ensamblado (los 2 forEach) o en el fallback del
  // «⋮». Si aparece uno fuera de esos sitios, alguien está pintando la cabecera a mano.
  const sueltos = apendices - enEnsamblado - (VISTA.match(/headRight\.appendChild\(overflowMenu/g) || []).length
    - (VISTA.match(/forEach\(\(b\) => headRight\.appendChild\(b\)\);\s*$/gm) || []).length;
  assert.ok(
    sueltos <= 0,
    `🔴 hay ${sueltos} \`headRight.appendChild\` fuera del ensamblado: una acción de cabecera que ` +
      'el registro no declara. Es como nace la segunda tabla.',
  );
});
