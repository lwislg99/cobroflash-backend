...
test('SCRUM-25: exports admin-only, CSVs nuevos, base+IVA, rango y audit', { skip: !ENABLED }, async (t) => {
  ...
  try {
    await withMerchant(prisma, { name: 'QA S25 A', email: `qa-s25-A-${stamp}@test.local` }, (merchantA) =>
      // vecino: sus datos NO pueden salir en el export de A
      withMerchant(prisma, { name: 'QA S25 B', email: `qa-s25-B-${stamp}@test.local` }, async (merchantB) => {
