...
const PENDIENTES_DE_FIRMA = [
  ...
  // SCRUM-915j · y de 909 a 932: la lista de clientes por botones y sus funciones se escriben ANTES de
  // esta línea (+23). CORREGIR un anclaje no es añadirlo: el texto es byte a byte el mismo y el motivo
  // sigue siendo el suyo. Cifra MEDIDA con el propio censo sobre el árbol resultante (21-sep-2026; el
  // rojo decía `quotesView.js:932`), no contada del diff.
  { fichero: 'public/dashboard/js/quotesView.js', linea: 932, texto: 'Solo presupuesto (facturación manual)',
