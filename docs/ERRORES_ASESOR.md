...
- **#12 y #13 — verificar de verdad, pero el objeto equivocado.** Un guard probado en rojo con un caso que caía *fuera* del mecanismo que vigilaba, y un job de CI cuyas piezas se comprobaron una a una sin ejecutarlo nunca. Aquí sí se comprobó; lo que falló fue **sobre qué**.
  ...
  >
  >    **Y en prosa no hay AST.** Al extender ese guard a los `.md` (SCRUM-233, 30-jul), el diseño obvio —la herramienta de recorte cerca de `.env`— habría cazado dos documentos que **prohíben** el defecto: `docs/MIGRATIONS_PENDING.md` («el `sed` se lleva la comilla del `.env`») y **este mismo fichero**, en la regla derivada de #9. Sin árbol sintáctico el discriminador no puede ser el ámbito: tiene que ser la **forma**. Las dos menciones legítimas escriben la herramienta **desnuda**, y lo que un lector copia y teclea es lo **invocable**, con sus flags. **Nombrar no es enseñar; invocar sí.** De ahí lo que de verdad se aprende, y sirve para cualquier prohibición escrita: **para prohibir una forma peligrosa no hace falta escribirla** — se nombra la segura y se deja que el guard recuerde la mala, que para eso corre. Este párrafo es su propia prueba: describe el defecto entero sin contener un solo comando copiable, y por eso el guard lo lee sin necesitar ninguna excepción.
  >
  >    *(Las dos notas son la misma cara en dos capas del repo: la de arriba, en un hook que ejecuta; la de abajo, en la prosa que lo documenta. El hook sigue siendo de texto — es lo único de las dos que queda sin arreglar.)*
