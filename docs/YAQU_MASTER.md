...
- **R19 · Rotar la contraseña de la BD de PRODUCCIÓN (Railway):** procedimiento completo en `docs/RUNBOOKS.md`. Tres puntos que no son evidentes y que deciden el resultado: ① **cambiar `POSTGRES_PASSWORD` en Railway NO rota la contraseña** (la imagen solo la lee al inicializar el volumen) — hay que hacerlo dentro de Postgres con `\password`, que además hashea en el cliente y no deja la clave en claro en los logs; ② **`/health` en verde NO prueba que la credencial nueva funcione**: Postgres autentica al ABRIR la conexión, así que el pool ya abierto sigue respondiendo `db: up` con la contraseña vieja — la verificación exige arranque en frío; ③ **no hay rotación sin corte** sobre el mismo rol (Postgres no admite dos contraseñas por rol): ventana de 1-3 min en la que fallan las conexiones NUEVAS, y prohibido mergear a `main` durante ella (auto-deploy = arranque en frío con las variables a medias). La vía sin corte (rol nuevo) se descarta a propósito: las tablas las posee `postgres` y `ALTER TABLE` exige propiedad, así que rompería el siguiente `db push`. Nace del incidente #14 de `ERRORES_ASESOR.md` (fuga real, credencial rotada).
...
- **SCRUM-195 · `Job` 1:N `Quote`, paso 1 (código) + el parseo de URL de BD que no puede filtrar. 🟡 PARCIAL (código ✅, BD 1/3).** Mecanismo 2 de SCRUM-37 (presupuesto adicional sobre un trabajo en curso). Se separó del ticket padre porque lo que bloqueaba a SCRUM-37 era una propuesta de cambio de máster (regla 27), y esto es la parte que no la necesita.
  · **La bomba que se desarma.** `recalcJobCobradoForQuote` resolvía el Job por `quoteId`, agregaba las facturas de ESE quote y **sobrescribía** `Job.totalCobrado`. Con 1:1 daba igual; en cuanto un Job tenga dos quotes, cobrar un adicional de 200 € sobre un Job con 3.000 € cobrados dejaría `totalCobrado = 200` — **el total BAJA después de cobrar**, y es el número con el que el pro sabe cuánto le deben. Segundo síntoma: un adicional sin Job apuntándole daba `findUnique` → `null` y la función se iba **en silencio**. Es el patrón de SCRUM-141 y `vat_default`: un agregado materializado que deja de cuadrar con lo que agrega. Ahora el núcleo es `recalcJobCobradoForJob(jobId)` y agrega con `quoteId: { in: quotesDelJob(jobId) }`. **Con 1:1 el resultado es idéntico**, así que era seguro aplicarlo hoy, antes de que el schema abra.
  · **`quotesDelJob` es el ÚNICO punto que resuelve la pertenencia**, y aguanta las TRES etapas de la migración a la vez: ① `findMany` por `Quote.jobId` (la vía nueva) ② dentro de un `try` que cae a `Job.quoteId` **tanto si Prisma lanza como si devuelve vacío**. Las dos caídas son necesarias y cada una cubre una ventana REAL: el `catch` cubre que **producción no tiene la columna y Railway despliega solo desde `main`**, o sea que este código llega allí ANTES que su schema; el vacío cubre el hueco entre aplicar la columna y correr el backfill. Sin ellas, el merge congela el cálculo del dinero en prod **sin ruido**, porque el recálculo es best-effort y se traga el error. El paso 2 retira `Job.quoteId` y con él la rama ②.
  · **Guard de conciliación** que pidió el fundador: `totalCobradoCuadra(job, invoices)` — nadie comprobaba que el materializado coincidiera con la suma de las facturas que dice agregar. Un materializado sin conciliación miente indefinidamente sin que nada falle.
  · **Schema (aditivo):** `Quote.jobId Int? @map("job_id")` + `@@index([merchantId, jobId])`. Nullable a propósito; `Job.quoteId @unique` NO se retira (es el paso 2). **SIN FK, decisión del fundador** — la integridad `Quote.jobId → Job.id` **la sostiene el código**, y el servicio de borrado de SCRUM-192 es quien la respeta al borrar. Medido antes de tocar nada: staging 3 pares / 0 rotas / 0 duplicados; producción 42 pares / 0 `quote_id` nulo / 83 quotes sin job (los no aceptados) / 0 rotas / 0 duplicados. **Aplicado solo a staging**; `yaqu_dev_javier` y producción pendientes con su preview y GO aparte (orden de SCRUM-169; las tres en `MIGRATIONS_PENDING.md`).
  · **🚨 FUGA DE CREDENCIAL, y el mecanismo que la sigue.** Durante la medición, un script del scratchpad leyó `DATABASE_URL` del `.env` **sin quitarle las comillas** y llamó a `new URL(...)` a pelo: el error volcó la URL de PRODUCCIÓN **con su contraseña** a la salida del comando. Credencial rotada por el fundador (regla 9). Lo medido después, en Node 24, corrige la intuición de todos: **`new URL()` NO pone la cadena en `.message`** (dice solo «Invalid URL») — viaja en `e.input`, y lo que la publicó fue el **volcado del objeto**. O sea que el reflejo de `console.error(e.message)` no habría evitado nada. Igual con `execFileSync`: en ENOENT el mensaje sale limpio y la URL va en `.spawnargs`; solo con salida ≠ 0 aparece en `.message` («Command failed: … argv»). **Mecanismo:** `parseBDSegura()` (tolera las comillas del `.env`, devuelve solo `host`/`base`/`usuario`/`puerto`, y su `catch` es **ciego a propósito** — sin binding no hay error que imprimir) y `redactarSecretos()` (acepta **objetos**, los inspecciona, tapa la contraseña y **deja el host**: redactarlo también dejaría el error inútil y empujaría a quitar el redactor). Viven en `scripts/_db-guard.mjs`, que ya es por donde pasa todo el que mira una URL de BD.
  · **Hallazgo colateral, y el peor sitio posible:** `scripts/backup-dump.mjs` pasa la `DATABASE_URL` en argv de `pg_dump` y su catch imprimía el error. Si `pg_dump` falla —versión, red, permisos, o simplemente no instalado— **el script de backup publica la contraseña de producción**, y un backup corre contra prod por definición. Redactado en los dos catches. `scripts/seed-video.mjs` hacía `new URL(dbUrl)` a pelo (seguro hoy, pero se atragantaba con las comillas): migrado a `parseBDSegura`.
  · **Lo que este guard NO puede hacer, dicho en el propio fichero:** el script que filtró vivía en el scratchpad, fuera del repo. Ningún test lo habría parado y este tampoco pararía al siguiente. Lo que hace es quitar la excusa —el helper existe y está en el camino— e impedir que el repo vuelva a la forma insegura. Regla operativa que acompaña: **un script que toca una BD importa `_db-guard.mjs`**; nunca `new URL(dbUrl)` suelto, ni «solo para leer el host».
  · Tests: `tests/scrum37b-agregacion-por-job.test.mjs` (11, prisma inyectado — esta ruta de DINERO no era verificable sin BD hasta ahora) y `tests/scrum195-url-bd-sin-fuga.test.mjs` (9). Verificados EN ROJO con 7 roturas realistas. **Dos de mis guards dieron verde falso en esa pasada y hubo que rehacerlos**: uno se conformaba con un redactor que *perdía la información entera* (exige el host ahora), y otro comprobaba `catch {` en todo el fichero y lo satisfacía el `catch` de **otra función** (acotado al cuerpo de `parseBDSegura`).
