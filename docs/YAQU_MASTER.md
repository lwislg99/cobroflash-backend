...
> **✅ SCRUM-33 · PAGOS-FLEX, etiqueta del tramo en factura (23-jul-2026, parte factura — PARCIAL):**
...
> o justificantes J-. Riesgo real: prácticamente nulo. **Cambio:** `Charge.concept` (creado en
> `invoiceWhatsApp.service.ts:60`) pasa de `"Factura 2026-CF-001"` a `"Factura 2026-CF-001 —
> Anticipo"` cuando `stageLabel` existe (null en presets → se omite, mismo patrón condicional que ya
> usa el resto del código) — se propaga solo con el valor del campo a los 6+ sitios que ya
> renderizan `Charge.concept` (recibo público, pago transferencia/Bizum/tarjeta/MP, bot WhatsApp),
> sin tocar cada uno. PDF de factura (`pdf.service.ts`, `generateInvoicePdf`): nuevo param
> `stageLabel?`, impreso en la cabecera junto a Nº/Fecha (nunca mezclado con el concepto de cada
> línea) — propagado en los 3 call-sites (`ensureInvoicePdf`, el flujo charge→factura de
> `lib/invoicing.ts`, `POST /admin/invoices/:id/regenerate-pdf`). **PARCIAL a propósito:** la parte
> de WhatsApp (nueva variable `{{5}}` en `payment_request_es`/`payment_confirmation_invoice_es`,
> plantillas YA aprobadas por Meta) exige re-aprobación — spec preparada, pendiente de que el
> fundador la envíe a Meta Business Manager; el código que la consume no se despliega hasta la
> aprobación. `npm test`: 207 · 178 pass · 0 fail · 29 skip. Sin schema.
