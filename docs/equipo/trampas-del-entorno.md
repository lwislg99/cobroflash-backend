...
| `grep -c $'\r'` cuenta **todas** las líneas aunque no haya ni un CR | MSYS se come el CR y queda un patrón vacío, que casa con todo | contar con node: `(fs.readFileSync(f,'latin1').match(/\r/g)\|\|[]).length` | un fichero fabricado con un CR tiene que dar 1, y uno limpio 0 |
