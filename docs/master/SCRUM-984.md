...
Sobre `092ccb5a`: la ficha del presupuesto aceptado (`quotesDetailView.js`, «Siguiente paso») ofrecía «Cobrar ahora» / «Ver justificante» / «Ver cobro pendiente» y **ningún camino al albarán**; y `GET /admin/quotes/:id` **no devolvía ningún dato de Trabajo** (`getQuoteDetailAdmin` de `quoteAdmin.ts` es una proyección explícita). El registro (`quoteActionsRegistry.js`) declaraba `btnCrearTrabajo` como primaria de `accepted` mientras la pantalla nunca lo pintó, y el Trabajo ya nace al aceptar (`ensureJobForQuote`, desde el 5-jul-2026).
...
5. **El primer CI cayó por DOS fallos míos que mi tanda focalizada no podía ver**, porque la seleccioné por «los ficheros que toco» y estos dos guards barren `tests/` ENTERO: `scrum262-telefonos-de-prueba` (mi fixture llevaba `34600000000`, un móvil que puede ser de alguien; el rango permitido es `340…`) y `scrum710b-anclaje-por-identidad` (un `albaranesView.js:150` en un mensaje: un número de línea es una posición y caduca). Los dos se arreglan en MI código (regla 41), sin tocar los guards; los números de línea se quitan también de este expediente. Aprendizaje: **todo fichero de test NUEVO tiene que pasar los guards que barren `tests/`** (262, 710b y el resto) antes de empujar, y eso no lo cubre «los tests de mis ficheros». Suite completa en CI: 7.889 tests, 7.794 pass, 93 saltados, **2 fail = estos dos**.
