...
**Medido contra:** `origin/main` = `ee082403e1aa37fad9c2ed485e368c9b147b1760` · 2026-08-05T11:10:53+02:00
...
### El rojo 4 no salió a la primera, y eso es el hallazgo
...
Reescrito para revisar **cada línea** contra las formas admitidas, en vez de cuadrar un número. Un
guard que cuenta se rompe en silencio; uno que enumera lo que encontró, no. Si no se hubiera
probado en rojo, este PR habría entregado un guard decorativo en el punto exacto que el ticket
pedía proteger.

## Verificación

- `npm run build` OK y `npm test` en **esa misma tirada**: **1572 tests · 1505 pass · 0 fail · 67
  skipped**.
- Guards vecinos que el cambio podía romper: SCRUM-274 (SHELL del service worker, 3/3 — el script
  nuevo entra en `sw.js`) y SCRUM-366 (la escalera, 5/5).
- Re-medido tras el rebase: `origin/main` se movió dos veces durante el ticket y en la segunda
  **entró SCRUM-366**, así que la rama dejó de ir apilada. La tanda de arriba es la de después.
