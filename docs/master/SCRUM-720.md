...
> ⚠️ **ESTE FICHERO TIENE DOS TRABAJOS DISTINTOS QUE COMPARTIERON NÚMERO, y no fue culpa de
> ninguna de las dos sesiones.** El mismo `SCRUM-720` se repartió a varias a la vez, y el guard de
> SCRUM-273 —con razón— exige un fichero por ticket: estábamos obligadas a escribir en el mismo
> sitio. Se conservan **los dos bloques** porque los dos son ciertos y están fechados.
>
> · **SCRUM-720b** — «Partes por valorar» no abría: `opts` donde el parámetro se llama `options`.
>   Carril del router del dashboard. **Aquí no se ha tocado ni una palabra de su contenido.**
> · **SCRUM-720** — la pantalla del parte no tenía una sola regla de CSS propia. Carril de la hoja.
>
> Desde el 4-sep cada encargo lleva su propio número, así que esto no debería repetirse.
