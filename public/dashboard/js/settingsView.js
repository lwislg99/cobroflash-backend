...
function renderSettingsView(container) {
    ...
    renderExportDataCard(container); // SCRUM-25 (S4): "Descargar mis datos"
...
// ── SCRUM-25 (EXPORT-1 · S4) · "Descargar mis datos" ────────────────────────
// Paquete ZIP con los CSV del negocio y los PDF de las facturas, para el asesor o
// una inspección. Solo Admin: Configuración ya es admin-only en el router de vistas,
// y el endpoint lo vuelve a exigir en BACKEND (requireRole + ADMIN_ONLY_ROUTES).
// La descarga va por navegación directa (es un fichero, no JSON): así el navegador
// gestiona el streaming y el usuario ve el progreso nativo.
function renderExportDataCard(container) {
  const card = document.createElement('div');
  card.className = 'customers-card';
  card.style.cssText = 'margin-bottom:20px';
  card.innerHTML = `
    <h2 style="margin:0 0 4px;font-size:18px;font-weight:700;color:var(--ink)">Descargar mis datos</h2>
    <p style="margin:0 0 14px;font-size:13px;color:var(--muted)">
      Un archivo ZIP con tus clientes, presupuestos, trabajos, cobros y facturas en CSV,
      más el PDF de cada factura. Listo para tu asesor.
    </p>
  ...
  const btn = card.querySelector('#btn-export-zip');
  const info = card.querySelector('#export-info');
  const inputFrom = card.querySelector('#export-from');
  const inputTo = card.querySelector('#export-to');
  let generando = false;
  ...
  // Aviso ANTES de pulsar: cuántas facturas caen en el rango y si pasa del tope.
  // El máximo lo dice el backend (MAX_FACTURAS_ZIP), no se duplica aquí.
    ...
    try {
      ...
      if (d.excede) {
        ...
        info.textContent = d.facturas === 0
          ...
          : `${base} en este rango (máximo ${d.maximo}). Preparar el archivo puede tardar hasta un par de minutos.`;
      }
    } catch {
