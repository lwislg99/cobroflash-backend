...
// SCRUM-814 · la invariante del tramo vive en UN sitio, compartida por los tres caminos.
...
router.post('/:id/invoice', requireRole('admin'), async (req, res) => {
  try {
    ...
    const invoice = await prisma.$transaction(async (tx) => {
      // SCRUM-814 · PRIMERA sentencia de la transacción: el tramo tiene que seguir libre. El
      // recuento va aquí dentro y bajo el cerrojo porque `stage` se eligió ARRIBA, fuera de la
      // transacción, y envolver la creación no protege una decisión tomada antes de abrirla.
    ...
    // queda intacta y volver a pedir da el tramo SIGUIENTE.
