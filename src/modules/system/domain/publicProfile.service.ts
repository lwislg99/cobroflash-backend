// src/modules/system/domain/publicProfile.service.ts — PERFIL-1 (A14.1, EXT3)
// Construcción PURA del HTML de la página pública /p/:slug (master Parte R). Extraído de
// publicProfile.routes.ts (SCRUM-124, r29 hermano) para poder probar por SEPARADO que el
// HTML nunca incluye datos sensibles — sin esto, la única forma de comprobarlo era leer el
// código a ojo, que es exactamente el patrón "prohibición sin mecanismo" que este ticket
// existe para cerrar (ver tests/scrum124-r29-perfil-publico-sin-datos.test.mjs).
//
// PÚBLICO: nombre comercial, logo, gremio, zonas (chips), años de experiencia, "Pedir
// presupuesto por WhatsApp" (wa.me del PRO), link de reseñas Google y footer "Hecho con
// YaQu". NUNCA público: precios, clientes, volumen, email, NIF, dirección exacta — esta
// función solo RECIBE los campos que puede necesitar (ver PublicProfileMerchant); si el
// caller le pasa un Merchant completo, los campos sensibles simplemente no se leen, pero
// el test de verdad es que el HTML no los contenga aunque se cuelen en el objeto.
...
export interface PublicProfileMerchant {
  name: string | null;
  logoUrl: string | null;
  trade: string | null;
  profileZones: unknown;
...
export function buildPublicProfileHtml(
  merchant: PublicProfileMerchant,
  opts: { slug: string; src: 'qr' | 'profile' },
): string {
  const { slug, src } = opts;
