...
export async function handleBotMessage(from: string, input: BotInput): Promise<boolean> {
  ...
  if (inIntake && text && isCancel(text)) {
    await sendWhatsAppText({
      merchantId,
 to: from, text: 'Sin problema, lo dejamos 👍' });
  ...
  if (session?.state === 'confirming_request') {
    ...
    if (wantsEdit) {
      ...
      await sendWhatsAppText({
      merchantId,
 to: from, text: '✏️ Vale, empezamos de nuevo. Cuéntame qué necesitas.' });
      return true;
    }
  ...
  if (listId === 'bot_pay') {
    ...
    if (!charges.length) {
      ...
      await sendWhatsAppText({
      merchantId,
 to: from, text: `🎉 ¡Estás al día! No tienes ningún pago pendiente con ${businessName}.` });
      await sendMenu(from, merchantId, businessName);
    } else {
  ...
  if (listId) {
    await sendWhatsAppText({
      merchantId,
 to: from, text: 'Ese menú ya caducó — te dejo el de ahora 👇' });
    await sendMenu(from, merchantId, businessName);
    await setSession(phone, {
      merchantId,
      state: 'menu',
