// srcNew/modules/quotes/app/routes/quotes.routes.ts
import { Router } from 'express';
import axios from 'axios';
import { prisma } from '../../../../core/db/prisma';
import { CreateQuoteSchema, AcceptQuoteSchema } from '../../../../core/validation/schemas';
import { calcTotal, makeReference, normalizePhone } from '../../../../core/utils/utils';
...
router.post('/:id/accept', async (req, res) => {
  try {
    ...
    const quote = await prisma.quote.findUnique({
      where: { id: quoteId },
      include: { customer: true, merchant: true, charge: true },
    });

    if (!quote) return res.status(404).json({ error: 'quote_not_found' });
    if (!quote.customer || !quote.merchant) {
      return res.status(400).json({ error: 'quote_missing_fk' });
    }
    ...
    if (body.send) {
      try {
        ...
        await axios.post(
          `${BASE_URL}/charges/${newCharge.id}/send`,
          override ? { to: override } : {},
        );
        sent = true;
      } catch (err) {
        console.error(
          'Auto-send after accept failed:',
    ...
    return res.json({
      ...
      sent,
      paybank_url: `${BASE_URL}/pay/bank/${newCharge.id}`,
      paycard_url: `${BASE_URL}/pay/card/${newCharge.id}`,
    });
  } catch (err: any) {
    if (err?.name === 'ZodError') {
      return res.status(400).json({
